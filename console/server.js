...
const API = (process.env.CHAOSMEDIC_API_URL || process.env.OPSGENTIC_API_URL || 'http://localhost:8000').replace(/\/$/, '');
...
async function proxy(res, method, path, body) {
  try {
    ...
    res.status(502).json({ error: `chaosmedic API unreachable: ${e.message}` });
  }
}
...
// ChaosMedic incident self-healing API
app.get('/api/incidents', (_req, res) => proxy(res, 'GET', '/api/incidents'));
app.post('/api/incidents', (req, res) => proxy(res, 'POST', '/api/incidents', req.body));
app.get('/api/incidents/:id', (req, res) => proxy(res, 'GET', `/api/incidents/${encodeURIComponent(req.params.id)}`));
app.post('/api/incidents/:id/approve', (req, res) => proxy(res, 'POST', `/api/incidents/${encodeURIComponent(req.params.id)}/approve`));
app.post('/api/incidents/:id/reject', (req, res) => proxy(res, 'POST', `/api/incidents/${encodeURIComponent(req.params.id)}/reject`));
app.get('/api/pipeline', (_req, res) => proxy(res, 'GET', '/api/pipeline'));
app.get('/api/memory', (_req, res) => proxy(res, 'GET', '/api/memory'));
...
app.listen(PORT, () => console.log(`chaosmedic-console on :${PORT} (api=${API}, ns=${NS})`));
