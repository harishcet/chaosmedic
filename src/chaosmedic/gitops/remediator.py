...
from chaosmedic.mcp.loader import explain_exception, load_connections
...
async def _run(state: dict) -> tuple[Optional[list], list]:
    ...
    from chaosmedic.mcp.agent_tools import summarize_tool_calls
...
def _auth_github(connections: dict) -> None:
    ...
    if not conn:
        return
    from chaosmedic.gitops.github_app import github_app_token
